...
country = cyber.security.1_enrolments %>% filter(country != "Unknown")
...
table_country=table(country$country)
country_1=sort(table_country,decreasing = T)[1:10]
class(country_1)
...
library(fmsb)
#可视化country_1
ggplot(country_1,aes(x=Var1,y=Freq,fill=country_1[,1],group=factor(1)))+geom_bar(stat="identity")
...
ggplot(employment_area,aes(x=Var1,y=Freq,fill=employment_area[,1],group=factor(1)))+geom_point(stat="identity")
